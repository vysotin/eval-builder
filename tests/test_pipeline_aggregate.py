...
def test_aggregate_pass_rates_thresholds_slices_and_stability():
    ...
    # stability: c1 outputs differ (agent), c3 same output but contract disagrees (evaluator)
    st = agg["stability"]
...
def test_output_hash_ignores_nothing_relevant():
    a = CaseRun(case_id="x", outputs={"response": "hi"}, tool_calls=[{"name": "t", "args": {"a": 1}}])
    b = CaseRun(case_id="x", outputs={"response": "hi"}, tool_calls=[{"name": "t", "args": {"a": 2}}])
    assert output_hash(a) != output_hash(b)
    assert output_hash(a) == output_hash(CaseRun(case_id="y", outputs={"response": "hi"}, tool_calls=[{"name": "t", "args": {"a": 1}}]))
