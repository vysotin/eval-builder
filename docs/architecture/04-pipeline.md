...
|---|---|---|---|
| preflight | – | `config.problems()`, `capability_check` (target importable), `provider_ready` for agent/generator/mock (blocking; mock only under `on_miss: llm`) and judge (degraded → problem); the config's `migrated` notes → problems; the deployment target's `available()` (docker daemon, `kubectl get nodes`, `oc whoami` — blocking); `deploy.env` names unset on the host → problems; details carry `deploy` (target, availability, image), `inference` and `evaluation` (workers, backend) | none |
