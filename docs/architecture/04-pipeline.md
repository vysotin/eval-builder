...
stages.py      the 16 stage functions + PipelineContext (lazy, disk-backed artifacts, the deployed agent)
...
overall_pass=0.8}`, `runs{repeats=3}`, `deploy{…}`, `inference{workers=4,
backend=threads|processes, timeout=120}`, `evaluation{workers=4, backend=threads|processes}`,
`mocking{required=true, on_miss=strict|llm|fallback|real, strategies=true, strategy=default,
on_invalid=fallback|strict, max_repairs=1}`, `stages{skip[], max_retries=1, simulate=true,
publish=auto}`, `review{auto_approve=false, approved_by, note}`, `output{dir}`,
`langsmith{dataset_name}`.

The `deploy` section says where the agent (with both mock layers) runs during
inference: `target=local|docker|kubernetes|openshift` (default `local`, a subprocess;
`pipeline init` writes `local` too, with the container targets in the comment),
`image{name=evalbuilder-<name>, tag=latest, registry, push=auto|registry|load|none,
extras[]}`, `build{context=".", dockerfile, include[], requirements}`, `port=8080`,
`namespace=default`, `replicas=1`, `expose=port-forward|nodeport|route`, `env{}` (a
`null` value copies the variable from the host — how API keys reach the container),
`keep=false`, `timeout=240`, `context`. `image_ref` (`[registry/]name:tag`) and
`container_target` (every target but `local`) are derived properties.
...
like `provider:model`, `auto_approve` requires `approved_by`, `inference.workers` /
`inference.timeout` / `evaluation.workers` ≥ 1) and, through `_deploy_problems()`, the
deployment ones: `deploy.port` within [1, 65535], `replicas` / `timeout` ≥ 1, `openshift`
and `push: registry` need `image.registry`, `expose: route` is OpenShift-only,
`expose` is meaningless for `local` / `docker`, and under a container target neither
`models.agent` nor the mock model (`models.mock`, or the generator when `on_miss: llm`
and no mock model is named) may be a `claude-cli:` spec — the `claude` binary is not in
the image. The pre-phase keys `runs.parallel_intents` / `parallel_scoring` /
`parallel_simulations` are migrated on load by a `model_validator`: `parallel_intents` →
`inference.workers`, `parallel_scoring` → `evaluation.workers`, `parallel_simulations`
is dropped (scenarios share the inference pool), an explicit new section wins, and every
rewrite is noted in `migrated` (excluded from `to_yaml`, reported by preflight as
problems). `guidance()` renders
...
after every stage and carries `data.problems` and `data.stopped_after`. `STAGE_NAMES`
(`config.py`) is the order: preflight, discover, map, mocks, dataset, review, verify,
...
|---|---|---|---|
| preflight | – | `config.problems()`, `capability_check` (target importable), `provider_ready` for agent/generator/mock (blocking; mock only under `on_miss: llm`) and judge (degraded → problem); the config's `migrated` notes → problems; the deployment target's `available()` (docker daemon, `kubectl get nodes`, `oc whoami` — blocking); `deploy.env` names unset on the host → problems; the probe runs through `ctx.deploy_runner()` (logged to `work/deploy/commands.log`) and is the only one per pipeline — the `deploy` stage passes `check_available=False`; details carry `deploy` (target, availability, image), `inference` and `evaluation` (workers, backend) | none |
...
| deploy | verify | `deploy.deploy_up` on the config's target with a `CommandRunner` logging to `work/deploy/commands.log`: render the files, build the image (container targets), distribute it, deploy, wait for `/health` → `deployment.json` (target, image, endpoint, resources, commands); a healthy deployment already recorded is reused; unset `deploy.env` names → problems; details: target, image, endpoint, expose, reused, push mode, health, seconds | none (blocking; the failed record is written) |
| infer | deploy | `ctx.agent()` — a `RemoteAgent` for the recorded endpoint after `deploy_status` confirmed it healthy (restarting a dead port-forward) — then `runs.repeats` × `inference.infer_dataset(agent, workers=inference.workers, backend=inference.backend, on_miss, strategy)`: every case is one joblib task (its turns replayed through the endpoint with the case's merged mock block; under `llm` the engine runs inside the server, seeded with the conversation's `mocks.history`), results keep dataset order; live progress (repeat, per-case completions, per-intent tallies, mock-call totals, endpoint, workers, backend) is written to `work/run-progress.json` after every case; per-layer totals land in the stage details (`mocking.calls`) with the last run's `execution`; invalid engine answers become a problem; a run where every case is an infrastructure error fails the stage | retry once |
| simulate | deploy, review (optional) | generator writes scenarios (optionally `mock_strategy`) → `scenarios.yaml`; `inference.simulate_scenarios` runs them against the same deployed agent with the dataset's mock block (a scenario's strategy selects the engine strategy; results carry `mock_calls` and a per-turn `log`) and the generator as the simulated user (an injected generator object, else `models.generator` as a spec so process workers build their own), `inference.workers` scenarios at a time, results in scenario order → `simulation.json`; violations mined into pending cases | never blocks the verdict |
| teardown | deploy (optional) | `deploy.deploy_down`: remove the deployment (compose project / Deployment + Service + loader DaemonSet + port-forward / local process), mark the record `down`; skipped by config when `deploy.keep` is true; runs after `simulate` so the agent is gone before scoring starts | never blocks the verdict (a failure is a problem) |
| score | infer | `evaluators.yaml` written; `score_run(workers=evaluation.workers, backend=evaluation.backend)` per run — contiguous case-run splits scored concurrently by joblib, rows/metrics/slices assembled in run order so the report is identical to a sequential pass → `results/score-report-<id>.json`; dead evaluators (all errors) reported; no scores at all → failure | retry once |
...
`REQUIRED_STAGES` — the ones whose failure makes the verdict `incomplete` — are
preflight … verify, `deploy`, `infer`, `score`, `aggregate`; `simulate`, `teardown`,
`publish` and `analyze` are optional. `--until deploy` stops after the deployment and
leaves the agent running for manual probing (`evalbuilder deploy status|logs|down`);
...
so they survive resumes. The generator model is created once per run
(`model_from_spec`); the agent and mock models run inside the deployed server, which
gets their specs through its environment. `ctx.deployment()` reads `deployment.json`;
`ctx.agent()` refuses to run without a record in status `up`, calls `deploy_status`
(which restarts a dead port-forward and saves the live facts back) and returns a
`RemoteAgent(endpoint, timeout=inference.timeout)` — every phase that talks to the agent
goes through it. `ctx.deploy_runner()` is the logged command runner the deploy,
teardown and status calls share.
...
metrics, slices, stability, the `mocking` summary (whose call totals come from the
`infer` stage, or from `run` in a directory written before the rename), a `deployment`
summary (target, image, endpoint, expose, status, push mode — not the whole record),
per-case rows, failing cases with judge comments, simulation, publication, analysis,
generator calls, the loaded config and `problems[]` (stage problems plus
failed/awaiting/dependency-skipped stages). `analysis_summary` is the compact view the
analyze stage sends to the generator. `run_pipeline` wires config → context → runner
(adding `teardown` to the skip set when `deploy.keep` is true, next to `simulate` and
`publish` by config) and always leaves a report behind even if the report stage itself
failed. `summary_text` is the terminal rendering, with a `deployment:` line.
...
coverage, evaluators, deployment (`deployment.json`, `evalbuilder/deployment/v1`, a
`final` kind written by the deploy and teardown stages), run, score_report, aggregate,
scenarios, simulation, analysis, pipeline_state, pipeline_report, pipeline_job. The
rendered deployment files, `commands.log`, `serve.log` / `port-forward.log` live under
`work/deploy/` — scratch, like the rest of `work/`.
...
in the setup page). `build_config` rejects an empty name / source / module. The form
carries the deployment and parallelism fields (`deploy_target`, `image_name`,
`image_registry`, `namespace`, `expose`, `keep`, `infer_workers`, `infer_backend`,
`eval_workers`; choice tuples `DEPLOY_TARGET_CHOICES`, `EXPOSE_CHOICES`,
`BACKEND_CHOICES`) into `deploy`, `inference` and `evaluation` and back.
...
- The agent runs where `deploy.target` says: a container target needs an agent (and
  mock) model that runs inside the image — an API-key provider with the key passed
  through `deploy.env`, or a scripted model — never `claude-cli`; the judges and the
  generator still run on the host and may use it.
