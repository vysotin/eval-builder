...
| `simulate PATH --scenarios F [--endpoint URL \| --deployment DIR] [--workers N] [--backend threads\|processes] [--timeout S] [--out D] [--mine/--no-mine] [--mock] [--on-miss P] [--mock-model SPEC]` | multi-turn simulations only — against a deployed agent (`--endpoint`, or the endpoint in `DIR/deployment.json`) or in-process (default), joblib-parallel; violations mined as pending cases; mocks are opt-in (`--mock`), unlike `infer --scenarios` | `inference.simulate_scenarios`, `agent_client.*` | invalid scenario file, unhealthy endpoint, no deployment record, bad `--backend` |
