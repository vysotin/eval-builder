...
| Docker target publishes one host port | `deploy.port` is published on the host as `deploy.host_port` (default: the same number, `8080:8080`); a busy host port fails `compose up` — set `deploy.host_port` (docker only) rather than changing the container port. |
